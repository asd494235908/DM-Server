...
module.exports = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (file.size > form.maxFieldsSize) {
      fs.unlink(file.path);
      return res.json({ 'status': -1, 'msg': '图片不能超过2M', result: '' });
    }
    ...
    if (extName.length == 0) {
      return res.json({ 'status': -1, 'msg': '只支持jpg和png格式图片', result: '' });
    }
    ...
    fs.rename(file.path, newPath, (err) => {
      if (err) {
        return res.json({ 'status': -1, 'msg': '图片上传失败', result: '' });
      } else {
        let uid = req.headers.uid
        let url = `${confg.domain}/serverImage/${imageName}`
        const str = `UPDATE users SET user_img='${url}' WHERE uid=${uid}`
        const sql_a = sql.Up_Mysql_Data(str)
        sql_a.then(() => {
          return res.json({ 'status': 200, 'msg': '图片上传成功', result: { url } });
        }).catch(() => {
          return res.json({ 'status': -1, 'msg': '图片上传失败', result: '' });
        })
